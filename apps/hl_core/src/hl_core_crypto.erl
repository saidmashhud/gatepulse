...
%% Reads master key from HL_MASTER_KEY env var (base64-encoded 32 bytes).
-module(hl_core_crypto).
...
%% Call after rotating HL_MASTER_KEY.
rotate_key() ->
    logger:info(#{event => key_rotation_started}),
    ok.

master_key() ->
    case os:getenv("HL_MASTER_KEY") of
        false ->
            binary:copy(<<0>>, 32);
        B64 ->
