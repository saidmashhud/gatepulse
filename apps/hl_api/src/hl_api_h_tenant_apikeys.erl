...
-module(hl_api_h_tenant_apikeys).
-export([init/2]).
...
handle(<<"POST">>, Req0, Opts) ->
    ...
    case hl_api_key_store:put(TargetTenant, Name, Scopes, CallerTenantId) of
        {ok, Token, KeyId} ->
...
handle(<<"GET">>, Req0, Opts) ->
    TargetTenant = cowboy_req:binding(tenant_id, Req0),
    Items = hl_api_key_store:list(TargetTenant),
...
handle(<<"DELETE">>, Req0, Opts) ->
    ...
    case KeyId of
        undefined ->
            ...
            {ok, Req0, Opts};
        _ ->
            case hl_api_key_store:delete(TargetTenant, KeyId) of
                ok ->
