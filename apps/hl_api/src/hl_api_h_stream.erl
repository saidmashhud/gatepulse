...
init(Req0, Opts) ->
    ...
    hl_stream_pubsub:subscribe(TenantId, Topic, self()),
    hl_delivery_metrics:inc_stream_clients(),
...
info({replay_missed, AfterEventId}, Req, Opts) ->
    ...
    case hl_store_client:list_events(TenantId, #{
        ...
        {ok, #{<<"items">> := Items}} ->
            lists:foreach(fun(Event) ->
                ...
                case hl_core_topic:matches(Topic, ETopic) of
...
info({hl_stream, _TenantId, Event}, Req, Opts) ->
    Topic   = maps:get(<<"topic">>, Event, <<>>),
    Pattern = maps:get(topic, Opts, <<"#">>),
    case hl_core_topic:matches(Pattern, Topic) of
        true  ->
            send_sse_event(Event, Req),
            TId = maps:get(tenant_id, Opts, <<>>),
            hl_delivery_metrics:inc_stream_events(TId, Topic);
        false -> ok
    end,
    {ok, Req, Opts};
...
terminate(_Reason, _Req, #{tenant_id := TId, topic := Topic}) ->
    hl_stream_pubsub:unsubscribe(TId, Topic, self()),
    hl_delivery_metrics:dec_stream_clients(),
    ok;
