...
%% Runs every HL_COMPACT_INTERVAL_SECS (default: 3600s = 1h).
%% Deletes segments older than HL_RETENTION_DAYS (default: 7 days).
-module(hl_compaction).
...
schedule() ->
    IntervalMs = hl_config:get_int("HL_COMPACT_INTERVAL_MS", ?INTERVAL_MS),
    erlang:send_after(IntervalMs, self(), compact).

do_compact(State) ->
    RetentionSecs = hl_config:get_int("HL_RETENTION_SECS", ?RETENTION_SECS),
    T0 = erlang:monotonic_time(millisecond),
    Deleted = case hl_store_client:compact(RetentionSecs) of
