...
HookLine stores all data in append-only segment files under `HL_DATA_DIR`.
...
rsync -av --progress $HL_DATA_DIR/ /path/to/backup/$(date +%Y%m%d-%H%M%S)/
...
> **Warning:** Stop HookLine before restoring. Starting over existing data without stopping first will corrupt the store.
...
- If `HL_DATA_DIR` is on a filesystem with snapshot support (ZFS, LVM), use
