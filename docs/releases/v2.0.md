# HookLine v2.0 — Enterprise-grade OSS

> **Breaking changes** are present. See [Migration Guide](#migration-guide).

Feature implementation status at time of release:

| Feature | Status |
|---------|--------|
| Scopes / RBAC | ✅ Shipped |
| Audit log | ✅ Shipped |
| Event filters & transformation DSL | ✅ Shipped |
| Web console | ✅ Shipped |
| Secrets encryption at rest (`HL_MASTER_KEY`) | ⚠️ API surface exists, full rotation workflow in progress |
| PostgreSQL backend | ❌ Non-GA — config key exists, not production-ready |
...
- **Secrets encryption at rest** ⚠️: `HL_MASTER_KEY` and `POST /v1/admin/rotate-secrets` API surface exist; full GA rotation/storage workflow is still in progress — do not rely on this for compliance without testing end-to-end
...
- **Optional Postgres backend** ❌: `HL_STORE_BACKEND=postgres` exists as a compatibility path, but the native production PostgreSQL backend is not complete — use the default C store in production

## Breaking Changes

1. **API keys without scopes** created before v2.0 are treated as having all scopes during a grace period (configure via `HL_LEGACY_KEY_GRACE=true`, default `true`). Set to `false` to enforce strict scopes.
...
1. **Set `HL_MASTER_KEY`** (32-byte hex or base64) before starting v2.0 — required when encryption is enabled:
   ```bash
...
export HL_POSTGRES_URL=postgres://user:pass@host:5432/hookline
# Run migrations:
psql $HL_POSTGRES_URL -f priv/sql/001_initial.sql
```
...
|----------|---------|-------------|
| `HL_MASTER_KEY` | (none) | AES-256-GCM key for secret encryption |
| `HL_STORE_BACKEND` | `c` | `c` or `postgres` |
| `HL_POSTGRES_URL` | (none) | PostgreSQL connection URL |
| `HL_LEGACY_KEY_GRACE` | `true` | Treat pre-v2 keys as full-scope |
...
```json
{
  ...
  "transform": [
    ...
    {"op": "set_field", "path": "data.source", "value": "hookline"}
  ]
}
```
