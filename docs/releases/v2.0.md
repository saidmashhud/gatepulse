...
- **Secrets encryption at rest**: Endpoint `secret` fields are encrypted with AES-256-GCM using `HL_MASTER_KEY`; `POST /v1/admin/rotate-secrets` re-encrypts all secrets on key rotation
...
- **Optional Postgres backend**: `HL_STORE_BACKEND=postgres` + `HL_POSTGRES_URL` to use PostgreSQL instead of the C daemon; schema migrations in `priv/sql/`
