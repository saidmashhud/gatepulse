...
- **Helm chart**: `helm/hookline/` — deploys API deployment + store StatefulSet with PVC; includes HPA and configmap
...
```bash
helm install hookline helm/hookline/ \
...
|----------|---------|-------------|
| `HL_NODE_NAME` | `hookline@hostname` | Erlang node name |
| `HL_COOKIE` | (required for cluster) | Erlang distribution cookie |
| `HL_JOB_LEASE_SECS` | `60` | Job lease duration before reap |

## Upgrade Notes

Single-node deployments require no changes. To enable clustering, set `HL_NODE_NAME` and `HL_COOKIE` consistently across all nodes and ensure Erlang distribution ports (4369 EPMD + dynamic) are open between nodes.
