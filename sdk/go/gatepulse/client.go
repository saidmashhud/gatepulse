// Package gatepulse provides a Go client for the GatePulse webhook delivery service.
//
// Usage:
//
//	client := gatepulse.New("http://localhost:8080", "my-api-key")
//	event, err := client.Events.Publish(ctx, gatepulse.PublishRequest{
...
// Client is the GatePulse API client.
type Client struct {
	baseURL    string
	apiKey     string
...
// New creates a GatePulse client.
...
func (e *APIError) Error() string {
	return fmt.Sprintf("GatePulse API error %d: %s", e.Status, e.Message)
}
